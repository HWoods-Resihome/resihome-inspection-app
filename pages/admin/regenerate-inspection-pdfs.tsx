/**
 * /admin/regenerate-inspection-pdfs  (admin only)
 *
 * Retrofits the reworked Q&A inspection PDF (summary-first + clickable photos)
 * onto existing COMPLETED 1099 Leasing Agent, Vacancy/Occupancy, and
 * Community/Visit inspections. Runs from the browser as the logged-in admin so
 * each call carries the session; sequential-ish with live progress. Keep the tab
 * open while it runs.
 */
...
type Item = { id: string; label: string; address: string };

export default function RegenerateInspectionPdfsPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const r = await fetch('/api/admin/regenerate-inspection-pdfs?list=1', { cache: 'no-store' });
  ...
  async function regenById(id: string): Promise<LogLine & { url?: string }> {
    for (let attempt = 0; attempt <= MAX_RETRY; attempt++) {
      try {
        const r = await fetch(`/api/admin/regenerate-inspection-pdfs?id=${encodeURIComponent(id)}`, { cache: 'no-store' });
        const d = await r.json().catch(() => ({} as any));
        const result = (d.results && d.results[0]) || {};
        if (r.ok && result.ok) return { id, ok: true, msg: 'ok', url: result.pdfUrl };
        if (r.status >= 500 && attempt < MAX_RETRY) { await delay(800 * (attempt + 1)); continue; }
        return { id, ok: false, msg: (result.error || d.error || `HTTP ${r.status}`).toString().slice(0, 200) };
      } catch (e: any) {
  ...
  async function run(limit?: number) {
    ...
    const list = (typeof limit === 'number' ? items.slice(0, limit) : items).map((i) => i.id);
    setRunning(true);
    ...
    const worker = async () => {
      while (!cancelRef.current) {
        ...
        const res = await regenById(list[idx]);
  ...
  async function regenerateOne() {
    ...
    const res = await regenById(id);
    if (res.ok) setOneLink(res.url || null); else setOneErr(res.msg);
    setOneBusy(false);
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <Head><title>Regenerate Inspection PDFs</title></Head>
      <div className="max-w-2xl mx-auto px-5 py-6">
        <h1 className="font-heading font-extrabold text-xl text-ink">Regenerate Inspection PDFs</h1>
        <p className="text-sm text-gray-600 mt-1 leading-relaxed">
          Re-renders the new summary-first PDF (with clickable photos) for every <b>completed</b> 1099
          Leasing Agent, Vacancy/Occupancy, and Community/Visit inspection. Refreshes the PDF in place —
          no status change, no email/ticket. Keep this tab open while it runs.
        </p>
        ...
        <div className="mt-5 border border-gray-200 rounded-lg bg-white p-4">
          ...
          {oneLink && (
            <div className="mt-3 text-xs">
              <span className="text-emerald-700 font-semibold">✓ Regenerated — </span>
              <a href={oneLink} target="_blank" rel="noopener noreferrer" className="text-brand hover:underline break-all">open the new PDF</a>
            </div>
          )}
        </div>
        ...
        {items === null ? (
          ...
          <>
            <div className="mt-5 flex items-center gap-3 flex-wrap">
              <span className="text-sm text-gray-700"><b>{total}</b> completed inspections found (1099 · vacancy · community).</span>
