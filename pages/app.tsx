...
export default function Home() {
  ...
  // Services access (per-user, from User Management) — gates the app switcher into
  // Services. Distinct from isAdmin: a non-admin with Services = Yes can switch.
  const [canServices, setCanServices] = useState(false);
          ...
          <div className="lz-head max-w-3xl mx-auto px-4 pt-2 pb-2.5">
            <div className="flex items-center justify-between gap-3 mb-2.5">
              ...
              <div className="flex items-center gap-3 whitespace-nowrap">
                ...
                {/* App switcher — hamburger → Inspections / Services / Properties.
                    Shown to anyone with Services access (per-user, from User
                    Management) OR any admin (so an admin without Services access
                    still reaches the admin-only Properties list). A native
                ...
                {(canServices || isAdmin) && (
                  <details className="relative group">
                    ...
                    <div className="absolute right-0 mt-1 w-44 bg-white rounded-xl shadow-lg border border-gray-200 z-40 overflow-hidden text-ink">
                      ...
                      {isAdmin && <Link href="/properties" className="block px-4 py-2.5 text-sm hover:bg-gray-50">Properties</Link>}
                    </div>
                  </details>
                )}
