/**
 * lib/geocodeResolve.ts — server-side address → coordinates resolution, shared by
 * the /api/geocode endpoint AND the record-creation flows (inspections + service
 * work orders) that stamp latitude/longitude at create time so the map never has
 * to geocode live.
 *
 * US-focused: prefer the property's stored coords, then the US Census geocoder,
 * then OpenStreetMap Nominatim (structured query). Every result is validated
 * against the state named in the address, so a loose match / stale stored coord
 * can't drop a pin in the wrong state. Results (incl. misses) are cached in-process.
 */
import { fetchPropertyCoords, fetchPropertyAddress, fetchCommunityFirstPropertyId } from '@/lib/hubspot';

export type Coords = { lat: number; lng: number; source: string };
...
/**
 * Resolve reference coordinates for an address and/or a HubSpot record id.
 * `propertyId` may be a Property (stored coords / its address) OR a Community
 * (falls back to the community's first property). Every candidate is validated
 * against the state named in `address`. Returns null when nothing resolves.
 */
export async function resolveCoords(input: { address?: string; propertyId?: string }): Promise<Coords | null> {
  const address = (input.address || '').trim();
  const propertyId = (input.propertyId || '').trim();
  if (address.length < 5 && !propertyId) return null;
  ...
  // 1) Property's stored coordinates (validated against the address's state).
  if (propertyId) {
    try {
  ...
  // 2) Geocode the property's OWN street address.
  ...
  // 3) Community id → its first property's coords, then that property's address.
  if (!coords && propertyId) {
    try {
  ...
  // 4) Geocode the passed address text.
