...
export async function fetchAnswersForInspection(inspectionRecordId: string): Promise<SavedAnswer[]> {
  ...
  const properties = [
    ...
    'section', 'location', 'answer_summary', 'answer_value', 'question_text',
  ...
  for (let i = 0; i < answerIds.length; i += 100) {
    ...
    for (const r of resp.results || []) {
      ...
      if (ans.answerType === 'rate_card_line') {
        // question_text holds the (possibly custom) full description; answer_value
        // holds the catalog labor short description. If they match (or question_text
        // equals the short), then there's no custom override.
        const questionText = p.question_text || '';
        const customDescription = questionText && questionText !== ans.answerValue
          ? questionText : '';
        ans.rateCardLine = {
          ...
          customLaborFullDescription: customDescription || null,
