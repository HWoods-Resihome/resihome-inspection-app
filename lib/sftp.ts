...
export interface SftpProbeResult {
  ok: boolean;
  configured: boolean;
  /** The session's working directory after login (the real SFTP root). */
  cwd?: string;
  /** Whether the configured SFTP_REMOTE_DIR exists ('d' / '-' / false). */
  configuredDir?: string;
  configuredDirExists?: string | false;
  /** Directory listings to help locate the correct remote path. */
  listings?: Record<string, string[]>;
  error?: string;
}

/**
 * Diagnostic probe — connects, reports the working directory, whether the
 * configured SFTP_REMOTE_DIR exists, and lists a few candidate folders so we
 * can discover the correct remote path. Read-only; never writes. Admin-only use.
 */
export async function probeSftp(): Promise<SftpProbeResult> {
  const cfg = readConfig();
  if (!cfg) {
    return { ok: false, configured: false, error: 'SFTP not configured (set SFTP_HOST / SFTP_USERNAME / SFTP_PASSWORD).' };
  }
  ...
  try {
    ...
    let cwd = '/';
    try { cwd = await sftp.cwd(); } catch { /* some servers don't support realpath */ }

    const configuredDirExists = await sftp.exists(dir).catch(() => false as const);

    // List the working dir, the SFTP root, and each ancestor of the configured
    // dir so we can see where the path diverges from reality.
    ...
    for (const c of candidates) {
      try {
        const list = await sftp.list(c);
        listings[c] = list.map((e: any) => `${e.type === 'd' ? '[d] ' : '    '}${e.name}`).slice(0, 60);
      } catch (e: any) {
        listings[c] = [`(could not list: ${String(e?.message || e).slice(0, 80)})`];
      }
    }

    return { ok: true, configured: true, cwd, configuredDir: dir, configuredDirExists, listings };
  } catch (e: any) {
    return { ok: false, configured: true, error: String(e?.message || e).slice(0, 220) };
  } finally {
    try { await sftp.end(); } catch { /* ignore close errors */ }
