...
DEPRECIATION / USEFUL-LIFE CAPS — these are NOT general caps on all items. Apply caps ONLY to the specific depreciable scopes listed below (flooring and paint). Tenant time in home (provided in House Details; assume ~12 months if unspecified) governs how much to depreciate the tenant's charge-back on cap-eligible items like paint and carpet.
  ...
  MULTIPLE PETS → REPLACE, DON'T CLEAN: when House Details report a "Last tenant pet count" greater than 1 (multiple pets) AND a room's photos show CARPET, prefer REPLACING the carpet AND pad over cleaning/steam-cleaning it — multiple pets typically mean odor/urine saturation into the pad that cleaning can't resolve. So if such a room has a carpet CLEANING / steam-clean / shampoo / pet-odor-treatment line (and no carpet replacement already), raise an "edit" adjustment to SWAP it to carpet replacement + pad (pass suggestedLineItemCode for the carpet replacement, and note that pad replacement is also needed), keeping the depreciation/flooring cap applied to the replacement. If carpet replacement is already on the room, just flag the redundant cleaning line for REMOVAL (per CARPET REDUNDANCY). Only apply this when the photos actually show carpet in that room; do not invent a replacement where there's no carpet.
...
DUPLICATES & SCOPE INTEGRITY — check for and flag:
  ...
  - CLEANING LEVELS (Level 1 vs Level 2): always prefer Level 1. When you suggest adding a cleaning line, suggest the Level 1 cleaning option, never Level 2. If the scope already contains BOTH a Level 1 AND a Level 2 cleaning line, raise a type "remove" on the Level 2 line and keep the Level 1 — never the reverse.
