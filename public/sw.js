...
self.addEventListener('install', (event) => {
  ...
  event.waitUntil((async () => {
    try {
      ...
      // Precache the FULL JS/CSS bundle for this build (public/sw-precache.json,
      // generated by scripts/sw-precache.mjs) so every route AND the lazily-loaded
      ...
      try {
        const manifestRes = await fetch('/sw-precache.json', { cache: 'no-store' });
        if (manifestRes && manifestRes.ok) {
          const data = await manifestRes.json();
          const list = Array.isArray(data && data.assets) ? data.assets : [];
      ...
      } catch { /* no manifest / offline at install — SWR still warms the cache later */ }
