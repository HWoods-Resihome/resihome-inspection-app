...
(async () => {
  ...
  const cover = h(Page, { size: 'LETTER' },
    h(View, { style: S.cover },
      V(S.coverPad,
        ...
        T(S.coverSub, 'How to use the app end to end — inspections, the camera & AI tools, scoping, review, approval, and reporting.'),
        V(S.coverRule),
  ...
  const p4 = SEC(
    ...
    LI('1099 Leasing Agent:', 'Guided photo & question walk — listing health, condition, HVAC, meters, trash.'),
    LI('Scope Rate Card:', 'The full turn scope — line items, quantities, vendors, pricing, tenant chargebacks.'),
    LI('Turn Reinspect QC:', 'Verify a vendor completed the scoped work, line by line, against the before photos.'),
  ...
  const p10 = SEC(
    ...
    T(S.intro, 'Before a Scope Rate Card can be submitted, it runs an AI Review — a second set of eyes on your scope.'),
    H2('What It Checks'),
    ...
    LI('Missing Info:', 'prompts you to add a photo, or to justify a missing cleaning or painting scope — so nothing’s skipped.'),
    H2('You’re in Control'),
    P('For each suggestion you can ', B('Edit'), ', ', B('Add'), ', or ', B('Ignore'), ' it — in real time. Nothing changes without you.'),
    CALL('GREEN MEANS GO', 'Once you’ve worked through it, the review turns green and Submit unlocks — a quick pass keeps quality high before a manager sees it.'),
  );
  ...
  const p11 = SEC(
    ...
    H2('The Flow'),
    STEP(1, 'You submit, and the inspection moves to Pending Approval.'),
    ...
    LI('Generates the PDFs:', 'master, vendor, and tenant chargeback.'),
    ...
    CALL('TWO SETS OF EYES', 'The person who submits a scope can’t finalize their own — a second reviewer must approve it. Every turn is double-checked.'),
  );
  ...
  const p12 = SEC(
    ...
    P('The overall result determines whether the property goes ', B('On-Market'), ' or stays ', B('Under Turnkey'), ' until failed lines are done. On submit, the maintenance coordinator is notified in real time.'),
