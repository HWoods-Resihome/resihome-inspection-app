  ...
  return (
    <div className="max-w-7xl mx-auto px-5 sm:px-6 md:pb-24">
      ...
      <div className="pt-3 pb-2">
        {/* Title + status chip + controls all on the SAME top line: title (with
            inspector + report link) fills the left; chip + buttons cluster right. */}
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            <FitText text={props.templateLabel} className="font-heading font-bold text-gray-900" max={22} min={11} copyLink={`/inspection/${props.inspectionRecordId}`} />
            {/* Inspector name — no "Inspector:" prefix so more of the name shows. */}
            <div className="text-xs text-gray-500 mt-0.5 truncate">{props.inspectorName}</div>
            {props.pdfUrl && (
              <a
                href={props.pdfUrl}
        ...
        return (
          <section key={s.key} className="mb-4 border border-gray-200 rounded-xl overflow-hidden shadow-md">
            <button
              ...
              className="w-full flex items-center justify-between px-4 py-2.5 bg-brand/5 border-b border-brand/20 text-left"
            >
              <div className="flex items-center gap-2 min-w-0">
                <span className={'text-gray-400 transition-transform ' + (isCollapsed ? '' : 'rotate-90')}>&#9654;</span>
                <h2 className="font-heading font-bold text-lg text-gray-900 truncate min-w-0 flex-1">{s.displayName}</h2>
                {hasLines && (
              ...
              <>
                <div className="px-4 py-3 grid grid-cols-2 gap-3 border-b border-gray-100">
                  ...
                  <div className={'rounded-lg border-2 p-2.5 min-w-0 ' + (failNeedsPhoto ? 'border-brand bg-brand/5' : 'border-teal-300 bg-teal-50/60')}>
                    ...
                    >
                      {!props.readOnly && (
                        // Full-width, ≥44px tap target so the hit area matches the
                        // box the user sees — a small inline button left dead space
                        // in the wide After tile that swallowed taps ("press below
                        // the button to actually add a photo").
                        <button
                          type="button"
                          onClick={() => setCameraKey(s.key)}
                          className="w-full flex items-center justify-center gap-1.5 text-xs font-semibold bg-teal-600 text-white rounded px-2 py-2.5 min-h-[44px] hover:bg-teal-700 whitespace-nowrap"
                        >
