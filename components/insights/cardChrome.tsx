...
export function CardFrame({
  ...
  // When rendered inside a <CardSlot>, the ENTIRE header row toggles this card's
  // collapse (body hides, header stays). The chevron is just an indicator; any
  // interactive control in headerRight stops propagation so it doesn't collapse.
  ...
  return (
    <div className="bg-[#18181c] rounded-xl border border-white/10 flex flex-col overflow-hidden">
      <div
        className={`flex items-center gap-2 px-4 py-3 ${collapsed ? '' : 'border-b border-white/10'} ${slot ? 'cursor-pointer select-none' : ''}`}
        role={slot ? 'button' : undefined}
        tabIndex={slot ? 0 : undefined}
        aria-expanded={slot ? !collapsed : undefined}
        onClick={slot ? slot.toggle : undefined}
        onKeyDown={slot ? (e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); slot.toggle(); } } : undefined}
      >
        ...
        {headerRight && !collapsed && <div className="shrink-0" onClick={(e) => e.stopPropagation()}>{headerRight}</div>}
        {slot && (
          <span className="shrink-0 text-[#71717a] -mr-1 p-1" aria-hidden="true">
