      ...
      for (const a of approved) {
        ...
        if (effType === 'remove' && a.lineExternalId) {
          ...
          const line: RateCardLineInput = {
            ...
            assignedTo: defaultVendorForItem({ lineItemCode: a.suggested.lineItemCode, description: a.suggested.description }) || a.suggested.assignedTo || 'Vendor 1',
            note: '',
            ...
            {(
              <div className="relative order-3">
                <button
                  ...
                  aria-label={props.readOnly ? 'History' : 'Settings'}
                  aria-expanded={showSettingsMenu}
                  title={props.readOnly ? 'History' : 'Settings'}
                  ...
                  <>
                    ...
                    <div className="absolute right-0 mt-1.5 z-50 w-52 rounded-xl border border-gray-200 bg-white shadow-lg ring-1 ring-black/5 overflow-hidden animate-[fadeIn_120ms_ease-out]">
                      ...
                      <button
                        ...
                        className={`w-full text-left px-3.5 py-2.5 text-sm text-gray-700 hover:bg-gray-50 flex items-center gap-2.5 transition-colors ${props.readOnly ? '' : 'border-t border-gray-100'}`}
            ...
            <div className="text-left min-w-0 flex-1">
              <div className="flex items-center gap-1.5">
                ...
                {props.pestControlEnrolled && (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src="/pest-control.png" alt="Pest control enrolled" title="Enrolled in pest control" className="h-5 w-5 shrink-0 translate-y-[2px]" />
                )}
              </div>
    ...
    >
      ...
      <div className="flex items-center gap-2 min-w-0 flex-wrap">
        ...
        <div className="flex items-center gap-2 min-w-0 flex-1">
          ...
          {/* Photo status — a FIXED-WIDTH slot with the camera left-aligned, so the
              camera sits at the SAME x on every row (it doesn't shift left when a
              count appears). Same-size SVG camera in both states: NEEDED = amber
              camera + amber asterisk (matches the required-field "*" convention);
