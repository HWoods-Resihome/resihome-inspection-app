  ...
  const sectionEls = renderSections.map((section) => {
    ...
    const header = props.bare ? (
      ...
      <button
        ...
        aria-expanded={sopen}
        className="w-full bg-brand/5 text-gray-900 border-b border-brand/20 px-4 py-3 flex items-center gap-3 text-left hover:bg-brand/10 transition"
      >
      ...
      <button
        ...
        onClick={() => setOpenSections((m) => ({ ...m, [section.id]: !(m[section.id] ?? true) }))}
        aria-expanded={sopen}
        className="w-full px-4 py-2.5 bg-brand/5 hover:bg-brand/10 border-b border-brand/20 flex items-center gap-2 text-left"
      >
