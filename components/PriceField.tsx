...
export function PriceField({
  ...
  muted?: boolean;               // grey text + grey fill — a NOT-editable / derived field
  ...
  const box = `flex items-center gap-1 w-full border rounded-lg px-2.5 py-2 ${highlight ? 'border-emerald-300 bg-emerald-50' : muted ? 'border-gray-200 bg-gray-100' : 'border-gray-300 bg-white'} ${readOnly ? '' : 'focus-within:border-brand'}`;
  const val = `flex-1 min-w-0 text-sm text-center tabular-nums ${highlight ? 'text-emerald-700 font-bold' : muted ? 'text-gray-400 font-semibold' : 'text-ink'}`;
