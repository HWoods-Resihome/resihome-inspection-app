...
export function QuestionForm({
  ...
  existingAnswers, readOnly, onFirstEdit, onCancelInspection, isAdmin,
  ...
  // Header gear dropdown (History + admin "Change Property") and the reassign modal.
  const [showHeaderMenu, setShowHeaderMenu] = useState(false);
  const [showChangeProperty, setShowChangeProperty] = useState(false);
  // "Change Property" is admin-only and property-backed (Community/Visit reference
  // a community, not a property — excluded).
  const canChangeProperty = !!isAdmin && templateType !== 'pm_community_inspection';
    ...
    <main className="min-h-screen bg-white">
      ...
      <div className="lz-head max-w-3xl mx-auto px-4 pt-3 pb-2">
        <div className="flex items-start justify-between gap-3">
          ...
          {/* Header gear — opens a small menu: Inspection history (audit trail,
              every status incl. completed/read-only) + admin "Change Property"
              (reassign to a different property). When there's no admin action it
              acts as a direct History button, preserving the one-tap common case. */}
          ...
          <button
            ...
            className="inline-flex items-center justify-center w-6 h-8 -mx-0.5 text-gray-400 hover:text-gray-700"
            ...
            <section id={sectionDomId} className="lz-gap mb-8 scroll-mt-24 rounded-xl shadow-md overflow-hidden">
              ...
              >
                <div className="flex items-center gap-3 min-w-0 flex-1">
                  ...
                  {/* NEEDED = amber camera + amber asterisk (same size + style as the
                      Rate Card); clears once a photo is added for the section. */}
                  {photosMissing && (
                    <span
                      className="inline-flex items-center gap-1 shrink-0 text-amber-500"
                      title="Photos needed — this section requires at least one photo"
                      aria-label="Photos needed"
                    >
                      <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="shrink-0"><path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z" /><circle cx="12" cy="13" r="4" /></svg>
