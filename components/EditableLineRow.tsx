 ...
 *           A small ⚙ icon at the right edge opens the modal for advanced edit
 *           (bid item overrides, full description editing, notes).
...
interface Props {
  ...
  onOpenAdvanced: (line: RateCardLineInput) => void;  // ⚙ icon → modal
}
...
export function EditableLineRow(props: Props) {
  const {
    ...
    onSave, onDelete, onDiscardNew, onOpenAdvanced,
  } = props;
  ...
  if (!isEditing) {
    ...
    return (
      <ViewRow
        ...
        onDelete={onDelete}
        onOpenAdvanced={() => onOpenAdvanced(line)}
  ...
  return (
    ...
    <tr ref={rowRef} className="border-b border-brand/30 bg-brand/5">
      ...
      <td className="px-2 py-1.5 align-middle min-w-[280px]">
        <Combobox
          compact
      ...
      <td className="px-2 py-1.5 align-middle min-w-[140px]">
        <select
          ...
          className="h-9 w-full border border-gray-300 rounded px-2 text-xs bg-white"
          title={vendor}
        >
      ...
      <td className="px-2 py-1.5 align-middle">
        <input
          ...
          className="no-spinner h-9 w-24 border border-gray-300 rounded px-2 text-sm text-right bg-white"
      ...
      {/* Tenant % (center-aligned) */}
      <td className="px-2 py-1.5 text-center align-middle min-w-[80px]">
        <select
          value={String(tenantPct)}
          onChange={(e) => setTenantPct(Number(e.target.value))}
          className="h-9 w-full border border-gray-300 rounded px-2 text-sm bg-white text-center"
        >
          {TENANT_PCT_OPTIONS.map((p) => <option key={p} value={p}>{p}%</option>)}
        </select>
      </td>
      ...
      <td className="px-2 py-1 align-middle">
        <div className="flex items-center gap-1">
          {selectedItem?.isBidItem && line && (
            <button
              type="button"
              onClick={(e) => { e.stopPropagation(); onOpenAdvanced(line); }}
              className="text-amber-600 hover:text-amber-800 text-base leading-none"
              title="Bid item — advanced labor/material overrides"
            >
              ⚙
            </button>
          )}
...
function ViewRow({ line, item, calc, readOnly, onEnterEdit, onDelete, onOpenAdvanced, onSaveDescription }: ViewRowProps) {
        ...
        <div className="flex items-center gap-1">
          {item.isBidItem && (
            <button
              ...
              className="text-amber-600 hover:text-amber-800 text-base leading-none px-1"
              title="Bid item — advanced labor/material overrides"
            >
              ⚙
            </button>
