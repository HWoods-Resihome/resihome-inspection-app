// Controlled modal for the admin "Change Property" action. Opened from an
// inspection form's existing header gear menu (NOT its own button). Reassigns
// the inspection to a different property (e.g. it was created against the wrong
...
// the completion email, then the record reloads to show the new details.
import { useEffect, useRef, useState } from 'react';
import type { Property } from '@/lib/types';
import { syncAllProperties, searchCachedProperties } from '@/lib/propertyCache';

type Step = 'search' | 'confirm' | 'working' | 'done' | 'error';

export function ChangePropertyModal({
  open, onClose, inspectionId, currentAddress, isCompleted, onDone,
}: {
  open: boolean;
  onClose: () => void;
  inspectionId: string;
  ...
  isCompleted: boolean;
  /** Called after a successful reassign so the caller can reload the record. */
  onDone: () => void;
}) {
  ...
  // Reset to a fresh search each time the modal opens, and warm the property
  // cache before the first query.
  useEffect(() => {
    if (!open) return;
    setStep('search'); setQuery(''); setResults([]); setSelected(null); setMessage(''); setResult(null);
    let cancelled = false;
    (async () => {
  ...
  // the new property + all property-derived details. The record GET reads
  // directly by id (strongly consistent), so the reload reflects the change.
  useEffect(() => {
    if (step !== 'done') return;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/40 p-4 pt-16 sm:pt-24" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
          ...
          {step === 'done' && result && (
            <div className="py-4">
              ...
              <div className="mt-4 flex justify-end">
                <button type="button" onClick={onDone} className="px-4 py-2 rounded-lg text-sm font-heading font-semibold bg-brand text-white hover:bg-brand-dark">View updated record</button>
              </div>
            </div>
          )}
          ...
          {step === 'error' && (
            <div className="py-4">
              ...
              <div className="mt-4 flex justify-end gap-2">
                ...
                <button type="button" onClick={onClose} className="px-4 py-2 rounded-lg text-sm font-heading font-semibold bg-gray-200 text-gray-800 hover:bg-gray-300">Close</button>
              </div>
            </div>
          )}
        </div>
